...
export default function ComplaintsPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      } else await loadComplaints('student');
    }
    init();
  }, []);
  ...
  async function submit(e: FormEvent) {
    ...
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) { setNotice('Please sign in before submitting a complaint.'); return; }
    const { error } = await supabase.from('complaints').insert({ user_id: user.id, title: title.trim(), category, location: location.trim(), details: details.trim() || null });
    if (error) { setNotice(`Could not submit complaint: ${error.message}`); return; }
  ...
  async function updateStatus(id: string, status: Complaint['status']) {
    ...
    if (error) { setNotice(`Could not update complaint: ${error.message}`); return; }
  ...
  return <main className="module-page">
    ...
    <div className="module-layout">
      <section className="card form-card"><h2>Create a complaint</h2><p className="muted">Give the campus team enough detail to act quickly.</p><form className="complaint-form" onSubmit={submit}>
